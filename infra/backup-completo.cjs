// backup-completo.cjs — backup COMPLETO antes do reset (migration 010)
//
// Baixa as 17 tabelas de conteúdo do Supabase e salva como JSON
// em C:\tempnext\infra\backups\ E em D:\ (HD externo).
//
// Diferente do backup-pre-007.cjs (que só cobria 3 tabelas), este cobre
// TODO o conteúdo gerado por usuários — necessário antes de um reset total.
//
// NÃO inclui os caches de infraestrutura (place_suggestions_cache, cep_index)
// porque o reset 010 também não os toca.
//
// Uso (no cmd, dentro de C:\tempnext\infra):
//   node backup-completo.cjs
//
// Pré-requisito: .env com SUPABASE_URL e a chave service_role.
// A chave service_role bypassa RLS — backup fica COMPLETO (todas as linhas).
// Com anon key o backup sai parcial. O script detecta e avisa.
//
...
// ── Tabelas de conteúdo (as 17 que o reset 010 vai apagar) ───
const TABELAS = [
  'profiles',
  'places',
  'place_profiles',
...
if (!SERVICE_KEY) {
  ...
  console.warn('        ATENÇÃO: backup parcial antes de um RESET é arriscado.');
}
...
(async () => {
  ...
  const nomePasta = `backup-${ts}-pre010-completo`;

  console.log('Backup completo pré-reset (migration 010) — 17 tabelas\n');
  ...
  for (const t of TABELAS) {
    ...
    try {
      ...
      totalLinhas += dados[t].length;
      console.log(`${dados[t].length} linhas`);
    } catch (e) {
      console.error(`FALHOU — ${e.message}`);
      console.error('\n[ERRO] Backup incompleto. NÃO rode o reset 010 sem um backup íntegro.');
      process.exit(1);
    }
  }
  ...
  for (const destino of destinos) {
    try {
      ...
      fs.writeFileSync(
        ...
        [
          'Backup COMPLETO pré-reset (migration 010_reset_dados)',
          'Data: ' + new Date().toISOString(),
          'Modo: ' + (SERVICE_KEY ? 'service_role (COMPLETO)' : 'anon (PARCIAL — RLS filtra)'),
          'Total: ' + totalLinhas + ' linhas em ' + TABELAS.length + ' tabelas',
          '',
  ...
  console.log(`\nBackup concluído: ${totalLinhas} linhas, ${salvos}/${destinos.length} destino(s).`);
  if (salvos < destinos.length) {
    console.log('(D: pode não estar conectado — verifique o HD externo.)');
  }
  if (salvos === destinos.length && SERVICE_KEY) {
    console.log('Backup íntegro e redundante. Seguro aplicar a migration 010.');
  }
})();
